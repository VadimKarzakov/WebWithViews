...
public class ModelServiceImpl implements ModelService {
    ...
    @Override
    public List<ModelCategoryDto> findPriceOfFigureByNickname(String brand_name) {
        return modelRepository
                .findNameAndCategoryBOrderByBrand(brand_name).stream()
                .map((s) -> modelMapper.map(s, ModelCategoryDto.class))
                .collect(Collectors.toList());
    }//извлекает список объектов ModelCategoryDto по названию бренда.
}
